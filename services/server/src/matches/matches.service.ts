...
export class MatchesService {
  ...
  async createCompleted(data: {
    ...
    // Auto-join both players to the XP ladder if they aren't already
    for (const userId of [data.challengerId, data.challengeeId]) {
      try {
        await this.leaderboardsService.xpJoin(userId, data.leaderboardId);
      } catch (error) {
        // Already joined — that's fine
        if (!(error as any).status || (error as any).status !== 409) {
          const msg = (error as Error).message || '';
          if (!msg.includes('already joined')) {
            console.error(`Failed to auto-join user ${userId} to XP ladder:`, msg);
          }
        }
      }
    }
